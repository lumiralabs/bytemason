...
When you run `berry new`, it clones the boilerplate that contains foundational logic.
...
2. **Generation database schemas and migrations**
    ```bash
    berry db setup ./spec/specification.json    # generate schemas and migrations
    berry db push                               # push the migrations to DB
    ```

2. **Generate code**
   berry code ./spec/specification.json
   
   # Fix any issues (runs automatically for the first time)
   berry repair
   ```
