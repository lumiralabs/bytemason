...
class FileContent(BaseModel):
    ...
    modify_strategy: Optional[str] = Field(None, description="Strategy for file modifications")

class GeneratedCode(BaseModel):
    files: list[FileContent] = Field(..., description="List of files to create/modify")
    dependencies: list[str] = Field(default_factory=list, description="Additional npm dependencies needed")
    errors: list[str] = Field(default_factory=list, description="Any generation warnings or errors")
